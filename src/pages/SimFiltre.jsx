...
export default function SimFiltre() {
  ...
  // Mevcut yıllar
  const yillar = [...new Set(cikmislar.map(s => s.yil))].sort((a, b) => b - a)

  // Seçili yılın sınavları
  const sinavlar = yil
    ? [...new Set(cikmislar.filter(s => s.yil === yil).map(s => s.sinav))]
    : []

  // Soru sayısı
  const sayi = (yil && sinav)
    ? cikmislar.filter(s => s.yil === yil && s.sinav === sinav).length
    : 0

  function basla() {
    const uygun = cikmislar
      .filter(s => s.yil === yil && s.sinav === sinav)

...
function DuzenleForm({ item, alanlar, onKaydet, onKapat, t, inline = false }) {
  ...
  if (inline) return <div className="flex flex-col gap-0 h-full">{icerik}</div>
...
function SoruKart({ item, onDuzenle, onSil, tip, t, secimModu, secili, onSec, aktif }) {
  return (
...
function FiltrePanel({ acik, onKapat, sekme, items, filtreler, setFiltreler, t }) {
  ...
  const kurullar = [...new Set(items.filter(i => !filtreler.donem.length || filtreler.donem.includes(i.donem)).map(i => i.kurulId).filter(Boolean))].sort()
  ...
  const sinavlar = [...new Set(items.filter(i => !filtreler.yil.length || filtreler.yil.includes(i.yil)).map(i => i.sinav).filter(Boolean))].sort()

  function toggle(key, val) {
    setFiltreler(f => ({ ...f, [key]: f[key].includes(val) ? f[key].filter(v => v !== val) : [...f[key], val] }))
  }
  function temizle() { setFiltreler({ donem: [], kurulId: [], ders: [], yil: [], sinav: [] }) }
  ...
  return (
    <AnimatePresence>
      {acik && (
        <>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            onClick={onKapat} className="fixed inset-0 z-30" style={{ background: 'rgba(0,0,0,0.4)' }} />
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b" style={{ borderColor: t.border }}>
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={onKapat} className="w-7 h-7 rounded-full flex items-center justify-center text-xs"
                  ...
                  <div className="flex gap-1.5 flex-wrap">
                    {donemler.map(d => (
                      <button key={d} onClick={() => toggle('donem', d)}
                        ...
                        style={{ background: filtreler.donem.includes(d) ? t.accent : t.bg3, border: `1px solid ${filtreler.donem.includes(d) ? t.accent2 : t.border}`, color: filtreler.donem.includes(d) ? '#FAF0D0' : t.dim }}>D{d}</button>
                    ))}
                  </div>
                </div>
              )}
                  ...
                  <div className="flex gap-1.5 flex-wrap">
                    {yillar.map(y => (
                      <button key={y} onClick={() => toggle('yil', y)}
                        className="px-3 py-1.5 rounded-lg font-display text-xs font-semibold"
                        style={{ background: filtreler.yil.includes(y) ? t.accent : t.bg3, border: `1px solid ${filtreler.yil.includes(y) ? t.accent2 : t.border}`, color: filtreler.yil.includes(y) ? '#FAF0D0' : t.dim }}>{y}</button>
                    ))}
                  </div>
                </div>
              )}
                  ...
                  <div className="flex gap-1.5 flex-wrap">
                    {sinavlar.map(s => (
                      <button key={s} onClick={() => toggle('sinav', s)}
                        className="px-3 py-1.5 rounded-lg font-display text-xs font-semibold"
                        style={{ background: filtreler.sinav.includes(s) ? t.accent : t.bg3, border: `1px solid ${filtreler.sinav.includes(s) ? t.accent2 : t.border}`, color: filtreler.sinav.includes(s) ? '#FAF0D0' : t.dim }}>{s}</button>
                    ))}
                  </div>
                </div>
              )}
                  ...
                  <div className="flex gap-1.5 flex-wrap">
                    {kurullar.map(k => (
                      <button key={k} onClick={() => toggle('kurulId', k)}
                        className="px-3 py-1.5 rounded-lg font-display text-xs font-semibold"
                        style={{ background: filtreler.kurulId.includes(k) ? t.accent : t.bg3, border: `1px solid ${filtreler.kurulId.includes(k) ? t.accent2 : t.border}`, color: filtreler.kurulId.includes(k) ? '#FAF0D0' : t.dim }}>{k}</button>
                    ))}
                  </div>
                </div>
              )}
                  ...
                  <div className="flex gap-1.5 flex-wrap">
                    {dersler.map(d => (
                      <button key={d} onClick={() => toggle('ders', d)}
                        className="px-3 py-1.5 rounded-lg font-display text-xs font-semibold"
                        style={{ background: filtreler.ders.includes(d) ? t.accent : t.bg3, border: `1px solid ${filtreler.ders.includes(d) ? t.accent2 : t.border}`, color: filtreler.ders.includes(d) ? '#FAF0D0' : t.dim }}>{d}</button>
                    ))}
                  </div>
                </div>
              )}
...
export default function Admin() {
  ...
  function tumunuSec() {
    setSeciliIds(seciliIds.size === filtrelenmis.length ? new Set() : new Set(filtrelenmis.map(i => i.id)))
  }
        ...
        <>
          ...
          {sekme === 'iceriak' ? (
            <div className="flex-1 flex overflow-hidden">
              <div className="flex-1 px-5 pb-6 overflow-y-auto flex flex-col gap-4 relative z-10 md:max-w-md">
              ...
              <div className="flex-1 flex flex-col min-w-0 md:max-w-md md:border-r relative" style={{ borderColor: t.border }}>
                ...
                <div className="px-5 pb-3 flex-shrink-0 flex gap-2 relative" ref={filtreRef}>
                  <input value={aramaFiltre} onChange={e => setAramaFiltre(e.target.value)}
                    placeholder="Ara..." className="flex-1 px-4 py-2.5 rounded-xl text-sm"
                    style={{ background: t.bg2, border: `1px solid ${t.border}`, color: t.text }} />
                  ...
                  <div className="absolute left-5 right-5 z-40" style={{ top: '100%' }}>
                    <FiltrePanel acik={filtreAcik} onKapat={() => setFiltreAcik(false)}
                ...
                {secimModu && (
                  <div className="px-5 pb-3 flex-shrink-0 flex items-center justify-between">
                    <button onClick={tumunuSec}
                      ...
                      style={{ background: tumSecili ? `${t.accent}20` : t.bg2, border: `1px solid ${tumSecili ? t.accent : t.border}`, color: tumSecili ? t.accent2 : t.dim }}>
                    ...
                    <span className="text-xs font-display" style={{ color: t.dim }}>{seciliIds.size} seçili</span>
                  </div>
                )}
                ...
                <div className="flex-1 px-5 pb-24 overflow-y-auto flex flex-col gap-2 relative z-10">
                  {yukleniyor ? (
                ...
                {duzenleItem ? (
                  <>
                    ...
                    <div className="flex-1 overflow-y-auto px-5 py-4">
                      <DuzenleForm item={duzenleItem} alanlar={alanlar} onKaydet={kaydet} onKapat={() => setDuzenleItem(null)} t={t} inline />
                    </div>
                  </>
                ) : (
                  <div className="flex-1 flex flex-col items-center justify-center gap-3">
                    ...
                    <p className="font-display text-sm text-center" style={{ color: t.dim }}>
                      Düzenlemek için bir kayda tıkla
                    </p>
                  </div>
                )}
              </div>
      ...
      <AnimatePresence>
        {duzenleItem && (
          <div className="md:hidden">
            <DuzenleForm item={duzenleItem} alanlar={alanlar} onKaydet={kaydet} onKapat={() => setDuzenleItem(null)} t={t} />
          </div>
        )}
      </AnimatePresence>
    </motion.div>

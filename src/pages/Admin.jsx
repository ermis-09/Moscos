...
function DuzenleModal({ item, alanlar, onKaydet, onKapat }) {
...
function SoruKart({ item, onDuzenle, onSil, tip }) {
  return (
    <div className="rounded-xl p-4 flex flex-col gap-2"
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-[9px] font-bold tracking-widest uppercase px-2.5 py-1 rounded-full"
          style={{ background: t.accent, color: '#FAF0D0' }}>
          {item.kurulId}
        </span>
        <span className="text-[9px] font-bold tracking-widest uppercase px-2.5 py-1 rounded-full"
          style={{ background: `${t.accent}20`, color: t.accent, border: `1px solid ${t.border}` }}>
          {item.ders}
        </span>
        {tip === 'cikmis' && (
          <span className="text-[9px] font-bold tracking-widest uppercase px-2.5 py-1 rounded-full"
            style={{ background: `${t.accent}10`, color: t.dim }}>
            {item.yil} · {item.sinav}
          </span>
        )}
        {tip === 'flash' && (
...
const t = temaAl('home', ayarlar)
  ...
  const filtrelenmis = items.filter(item => {
    ...
    return (
      item.kurulId?.toLowerCase().includes(filtre.toLowerCase()) ||
      item.ders?.toLowerCase().includes(filtre.toLowerCase()) ||
      item.soru?.toLowerCase().includes(filtre.toLowerCase()) ||
      item.onYuz?.toLowerCase().includes(filtre.toLowerCase())
    )
  })
        ...
        <>
          ...
          <div className="px-5 pb-3 flex-shrink-0">
            <input
              ...
              placeholder="Kurul, ders veya soru ara..."

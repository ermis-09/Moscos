...
async function kaydetVeCik() {
  if (kullanici) {
    try {
      console.log('kaydetmeye çalışıyorum', kullanici.uid)
      const soruIdleri = sorular.map(s => s.id)
      console.log('soruIdleri:', soruIdleri.length)
 ...
 async function kaydetmeyeCik() {
   if (kullanici) {
     try {
       await deleteDoc(doc(db, 'kullanici_aktif_sinav', kullanici.uid))
     } catch {}
   }
   setCikisModalAcik(false)
   navigate('/')
   ...
   >
     ...
     <Modal
       ...
       butonlar={[
         ...
         {
           label: 'Kaydetme ve Çık',
           stil: 'danger',
           onClick: kaydetmeyeCik,
         },
         {
           label: 'İptal',
           stil: 'ghost',
           onClick: () => setCikisModalAcik(false),
         },
       ]}
     />
   </motion.div>

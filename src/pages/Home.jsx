...
function RastgeleSoru({ sorular, t, kullanici }) {
 const [secilen, setSecilen] = useState(null)
 const [onay, setOnay] = useState(null)
 const [gunlukVeri, setGunlukVeri] = useState(null)
 const [seri, setSeri] = useState(0)
 const [enUzunSeri, setEnUzunSeri] = useState(0)
 const [yuklendi, setYuklendi] = useState(false)

 const gun = Math.floor(Date.now() / (1000 * 60 * 60 * 24))
 const soru = sorular.length > 0 ? sorular[gun % sorular.length] : null
 const harfler = ['A','B','C','D','E']

 useEffect(() => {
   if (!kullanici || !soru) { setYuklendi(true); return }
   async function yukle() {
     try {
       const snap = await getDoc(doc(db, 'kullanici_gunluk', kullanici.uid))
       if (snap.exists()) {
         const veri = snap.data()
         if (veri.tarih === bugunTarih()) {
           setGunlukVeri({ cevaplandi: true, dogruMu: veri.dogruMu, cevap: veri.cevap })
           setSecilen(veri.cevap)
         }
         setSeri(veri.seri || 0)
         setEnUzunSeri(veri.enUzunSeri || 0)
       }
 ...
 async function cevapla(harf) {
   if (secilen !== null || gunlukVeri?.cevaplandi) return
   setSecilen(harf)
   const dogruMu = harf === soru.dogruCevap
   if (!kullanici) return
   try {
     const snap = await getDoc(doc(db, 'kullanici_gunluk', kullanici.uid))
     ...
     const dun = dunTarih()
     let yeniSeri = 1
     if (eskiVeri.tarih === dun) yeniSeri = (eskiVeri.seri || 0) + 1
     else if (eskiVeri.tarih === bugun) yeniSeri = eskiVeri.seri || 1
     const yeniEnUzun = Math.max(yeniSeri, eskiVeri.enUzunSeri || 0)
     await setDoc(doc(db, 'kullanici_gunluk', kullanici.uid), {
       tarih: bugun, cevap: harf, dogruMu, seri: yeniSeri, enUzunSeri: yeniEnUzun,
     })
     setSeri(yeniSeri)
     setEnUzunSeri(yeniEnUzun)
     setGunlukVeri({ cevaplandi: true, dogruMu, cevap: harf })
   } catch (err) { console.error('cevapla hata:', err) }
 }

 if (!soru) return null

 const cevaplandi = secilen !== null || gunlukVeri?.cevaplandi
 const dogruMu = secilen === soru.dogruCevap

 const AlевIkonu = ({ width = 10, height = 12 }) => (
   <svg width={width} height={height} viewBox="0 0 10 12" fill="none">
     <path d="M5 0C5 0 8 3 8 6C8 7.5 7.2 8.8 6 9.5C6.5 8.5 6.3 7.2 5.5 6.5C5.5 7.5 4.8 8.3 4 8.8C4.2 8.2 4 7.5 3.5 7C3 7.8 2.5 8.8 2.5 9.5C1.5 8.7 1 7.5 1 6.2C1 4 2.5 2 3 1C3 2 3.5 2.8 4 3C3.8 2 4 1 5 0Z" fill="#FFA040"/>
     <path d="M5.5 6.5C5.5 7.5 4.8 8.3 4 8.8C4.2 8.2 4 7.5 3.5 7C3.2 7.5 3 8.2 3 9C2.2 8.3 2 7.3 2.5 6.2C3 5.5 4 5 4.5 4.5C4.3 5.2 4.5 6 5.5 6.5Z" fill="#FFD080"/>
   </svg>
 )
   ...
   <div className="rounded-2xl flex flex-col flex-1 overflow-y-auto" style={{ background: t.bg2, border: `1px solid ${t.border}` }}>
     <div className="flex items-center justify-between px-4 pt-4 pb-3 flex-shrink-0 border-b" style={{ borderColor: t.border }}>
       <div className="flex items-center gap-2">
         ...
         {kullanici && yuklendi && seri > 0 && (
           <span className="flex items-center gap-1 text-[9px] font-bold px-2 py-0.5 rounded-full"
             style={{ background: 'rgba(255,140,0,0.15)', color: '#FFA040', border: '1px solid rgba(255,140,0,0.3)' }}>
             <AlевIkonu />
             {seri}
           </span>
         )}
     ...
     {!kullanici && yuklendi && (
       <div className="px-4 pt-3 flex-shrink-0">
         <p className="text-[10px] italic" style={{ color: t.dim }}>Giriş yaparsan ilerlemen kaydedilir.</p>
       </div>
     )}
       ...
       {onay && !cevaplandi && (
         <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 4 }}
           ...
           <div className="flex gap-2">
             ...
             <button onClick={() => { cevapla(onay); setOnay(null) }} className="px-3 py-1.5 rounded-lg font-display text-xs font-semibold"
           ...
           <div className="flex items-center gap-4 rounded-xl px-4 py-3"
             ...
             <div className="text-center">
               <div className="flex items-center justify-center gap-1">
                 <AlевIkonu width={14} height={17} />
                 <p className="font-display text-xl font-bold leading-none" style={{ color: '#FFA040' }}>{seri}</p>
               </div>
               <p className="text-[9px] font-bold tracking-widest uppercase mt-1" style={{ color: '#FFA04080' }}>Güncel Seri</p>
             </div>
             <div className="w-px h-8 flex-shrink-0" style={{ background: 'rgba(255,140,0,0.2)' }} />
             <div className="text-center">
               <p className="font-display text-xl font-bold leading-none" style={{ color: '#FFA040' }}>{enUzunSeri}</p>
               <p className="text-[9px] font-bold tracking-widest uppercase mt-1" style={{ color: '#FFA04080' }}>En Uzun</p>
             </div>
             {seri === enUzunSeri && seri > 1 && (
               <div className="ml-auto">
                 <span className="text-[9px] font-bold px-2.5 py-1 rounded-full"
                   style={{ background: 'rgba(255,140,0,0.2)', color: '#FFA040' }}>Rekor! 🏆</span>
               </div>
             )}
           </div>
         </motion.div>
    ...
    <div className="flex flex-col gap-1.5">
      ...
      <div className="h-1.5 rounded-full overflow-hidden" style={{ background: `${color}20` }}>
        <motion.div className="h-full rounded-full" style={{ background: color }}
          initial={{ width: 0 }} animate={{ width: `${yuzde}%` }} transition={{ duration: 0.8, delay: 0.1 }} />
      </div>
    </div>
  )
}
  ...
  return (
    <div className="flex flex-col gap-5 flex-1">
      ...
      <div className="flex flex-col gap-2 flex-1 overflow-y-auto">
        <span className="font-display text-[9px] font-semibold tracking-[0.22em] uppercase flex-shrink-0" style={{ color: t.accent }}>Son Sınavlar</span>
...
function FlashcardIstatistik({ flashcardlar, kurullarData, t, navigate }) {
  ...
  const dersler = Object.entries(
    flashcardlar.reduce((acc, f) => { acc[f.ders] = (acc[f.ders] || 0) + 1; return acc }, {})
    ...
    <div className="flex flex-col gap-5 flex-1 overflow-y-auto">
      {dersler.length > 0 && (
        <div className="flex flex-col gap-3">
          <span className="font-display text-[9px] font-semibold tracking-[0.22em] uppercase" style={{ color: t.accent }}>Kart Dağılımı</span>
...
function SimulasyonIstatistik({ sonuclar, cikmislar, t, navigate }) {
  ...
  return (
    <div className="flex flex-col gap-4 flex-1 overflow-y-auto">
      ...
      <span className="font-display text-[9px] font-semibold tracking-[0.22em] uppercase" style={{ color: t.accent }}>Tüm Arşiv</span>
      <div className="flex flex-col gap-2">
        {arsiv.map(([key, val]) => {
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    async function sonuclariYukle() {
      try {
        const q = query(collection(db, 'kullanici_sonuclari', kullanici.uid, 'sonuclar'), orderBy('tarih', 'desc'), limit(50))
          ...
          <main className="flex-1 flex overflow-hidden relative z-10 mt-20">
            <div className="flex-1 px-5 pb-6 flex flex-col gap-3 overflow-y-auto md:border-r md:max-w-md" style={{ borderColor: t0.border }}>
              ...
              <div className="rounded-2xl p-4 flex flex-col gap-3 flex-shrink-0"
                style={{ background: t0.bg2, border: `1px solid ${t0.border}` }}>
                <span className="font-display text-[9px] font-semibold tracking-[0.22em] uppercase" style={{ color: t0.accent }}>Genel Durum</span>
                <div className="grid grid-cols-3 gap-2">
                  {[
            ...
            <div className="hidden md:flex flex-1 flex-col px-8 py-4 gap-4 overflow-y-auto">
              ...
              {kullanici && profilSonuclar.length > 0 && (
                <div className="rounded-2xl p-4 flex flex-col gap-3 mt-2"
                  style={{ background: t0.bg2, border: `1px solid ${t0.border}` }}>
                  <span className="font-display text-[9px] font-semibold tracking-[0.22em] uppercase" style={{ color: t0.accent }}>Genel İstatistik</span>
                  <div className="grid grid-cols-2 gap-2">
                    {[
        ...
        <Page theme="sinav" ayarlar={ayarlar} decoNum="02" triangleColor={t1.triangle}>
          <main className="flex-1 flex overflow-hidden relative z-10 mt-20">
            <div className="flex-1 px-5 pb-6 flex flex-col gap-3 overflow-y-auto md:border-r md:max-w-md" style={{ borderColor: t1.border }}>
              ...
              <RastgeleSoru sorular={sorularData} t={t1} kullanici={kullanici} />
              <motion.button whileTap={{ scale: 0.98 }} onClick={() => navigate('/sinav/filtre')}
            ...
            <div className="hidden md:flex flex-1 flex-col px-8 py-4 gap-4 overflow-hidden">
              {kullanici ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center gap-3">
                  ...
                  <button onClick={() => navigate('/profil')} className="px-5 py-2.5 rounded-xl font-display text-sm font-semibold"
